...
export default async function handler(req, res) {
  ...
  try {
    ...
    const html = `<!DOCTYPE html><html lang="en"><head><meta charset="UTF-8"><title>${title} — Integrated Planning Partners</title><meta name="description" content="${summary}"><meta property="og:title" content="${title}"><meta property="og:description" content="${summary}"><meta property="og:type" content="article"><link rel="canonical" href="${base}/article.html?slug=${encodeURIComponent(slug)}"><script>if(!/googlebot|bingbot|crawler|spider|slurp|duckduckbot|baiduspider|yandexbot/i.test(navigator.userAgent))window.location.replace('${base}/article.html?slug=${encodeURIComponent(slug)}');<\/script></head><body><h1>${title}</h1><p>${summary}</p>${pubDate ? `<time>${pubDate}</time>` : ''}<div>${body}</div></body></html>`;
