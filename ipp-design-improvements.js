(function(){'use 
function fixNav(){
  var nav=document.querySelector('nav ul.nav-links,nav .nav-links,.nav-menu');
  if(!nav)return;
  var items=Array.from(nav.querySelectorAll('li'));
  if(!items.length)return;
  // Desired order
  var order=['what we do','structurereview','incomearc','ipp retirement','about us','contact us','architecture','advisor portal','begin your review'];
...
function fixLayers(){
  ...
  // Correct order: 01 Guaranteed Income, 02 Living Benefits/LTC, 03 Tax Elimination, 04 Protected Growth
...
function fixStructureReview(){
  ...
  // Hide the advisor report / view report button
  ...
  // Hide schedule meeting elements entirely
  btns.forEach(function(btn){
    var t=btn.textContent.toLowerCase();
  ...
  // Also hide any calendly embeds or schedule sections
  Array.from(document.querySelectorAll('[class*=schedule],[class*=calendly],[id*=schedule],[id*=calendly]')).forEach(function(el){
    el.style.display='none';
  });
  // Update completion message — replace any scheduling language with advisor contact message
  var allEls=Array.from(document.querySelectorAll('h1,h2,h3,h4,p,div,[class*=complete],[class*=success],[class*=thank],[class*=confirm],[class*=message]'));
  allEls.forEach(function(el){
    if(el.children.length>0)return;
    var t=el.textContent;
    if(t.toLowerCase().includes('schedule')&&(t.toLowerCase().includes('meeting')||t.toLowerCase().includes('call')||t.toLowerCase().includes('consultation'))){
      el.textContent='Your advisor will be in touch with you shortly.';
    }
    if(t.toLowerCase().includes('book a')&&t.toLowerCase().includes('meeting')){
      el.textContent='Your advisor will be in touch with you shortly.';
    }
  });
}
